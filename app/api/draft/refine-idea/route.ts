...
Refine raw invention ideas into structured, clear technical concepts for patent drafting.

Do not hallucinate.
Do not add unsupported technical details.
Do not invent components, steps, or benefits that the user has not stated.
Preserve the user's core idea, but rewrite it in cleaner and more structured technical language.
...
export async function POST(req: Request) {
  try {
    const { projectId, userId, idea } = await req.json();
    ...
    await supabase
      ...
      .update({
        stage: "idea_refinement_review",
        refined_idea: refinedIdea || output,
